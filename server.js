...
myIo.on('connection', (mySocket) => {
  ...
  mySocket.on('myAdminLogin', (myData) => {
    ...
    const myIsFirstAdmin = myRoom.admins.length === 1; // just pushed above
  ...
  // Admin: start/stop/set duration of the shared room timer
  mySocket.on('myTimerControl', (myData) => {
    ...
    if (action === 'stop') {
      ...
      myRoom.timer = newDur;
      myRoom.countdownReset = true; // will apply on next tick if running
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #myMsg { resize: vertical; min-height: 42px; }
  ...
  <div id="myMainLayout">
    ...
    <div>
      ...
      <div id="myAuthPanel" class="my-card">
        ...
        <div class="my-tabs">
          <button class="my-tab active" onclick="myShowTab('join')">Join as Participant</button>
          <button class="my-tab" onclick="myShowTab('admin')">Admin / Create Room</button>
        </div>
        ...
        <div id="myJoinPanel" class="active">
          ...
          <button class="my-btn my-btn-primary my-btn-full" onclick="myJoin()">Enter Room →</button>
        </div>
        ...
        <div id="myAdminSetupPanel">
          ...
          <button class="my-btn my-btn-dark my-btn-full" onclick="myLogin()">Create / Enter Room →</button>
        </div>
      </div>
      ...
      <div id="myAdminDashboard">
        ...
        <div id="myShareBar" style="display:none; background:#161b22; border:1px solid #30363d; border-radius:8px; padding:10px 12px; margin-bottom:12px; align-items:center; gap:8px; flex-wrap:wrap;">
          ...
          <button id="myCopyUrlBtn" onclick="myCopyRoomUrl()" style="padding:5px 12px; background:#30363d; border:none; border-radius:5px; color:#c9d1d9; font-family:'Syne',sans-serif; font-size:0.75rem; font-weight:700; cursor:pointer; white-space:nowrap;">Copy</button>
        </div>
        ...
        <div id="myTimerControls">
          ...
          <button id="myTimerToggleBtn" class="my-admin-btn my-admin-btn-blue" onclick="myToggleTimer()" style="padding:5px 12px; font-size:0.78rem;">▶ Start</button>
          ...
          <button class="my-admin-btn" style="background:#30363d; color:#c9d1d9; padding:5px 12px; font-size:0.78rem;" onclick="mySetDuration()">Set</button>
        </div>

        <!-- AI Notice (shown if AI unavailable) -->
        <div id="myAiNotice">
          ⚠️ Chrome Built-in AI not detected. Enable it via
          <a href="javascript:void(0)" onclick="myCopyFlagsLink()">chrome://flags/#prompt-api-for-gemini-nano</a>
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:10px;">
          <button id="myTriggerBtn" class="my-admin-btn my-admin-btn-orange" onclick="myManualAI()">
            ▶ Run AI Now
          </button>
          <button class="my-admin-btn my-admin-btn-blue" onclick="myInitAI()">
            ↺ Init AI
          </button>
        </div>
        ...
        <div id="myReviewArea">
          ...
          <div style="display:flex; gap:8px;">
            <button class="my-admin-btn my-admin-btn-green my-admin-btn-full" onclick="myApproveSummary()">
              📢 Broadcast to Room
            </button>
            <button class="my-admin-btn" style="background:#30363d; color:#c9d1d9;" onclick="myDismissReview()">
              Discard
            </button>
          </div>
        </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center;">
          ...
          <button class="my-admin-btn" style="background:#161b22; color:#586069; padding:4px 10px; font-size:0.72rem;" onclick="myToggleLog()">Toggle</button>
        </div>
        <div id="myLogArea"></div>

        <!-- Danger Zone -->
        <div id="myDangerZone">
          <button class="my-admin-btn my-admin-btn-red my-admin-btn-full" onclick="myShutdownRoom()">
      ...
      <div id="myInputArea">
        <div class="my-input-row">
          <textarea id="myMsg" class="my-input" placeholder="Share your idea… (Ctrl+Enter to send)" style="resize:vertical; min-height:42px; line-height:1.4;" onkeydown="if((event.ctrlKey||event.metaKey)&&event.key==='Enter') mySend()"></textarea>
          <button class="my-btn my-btn-primary" onclick="mySend()">Send 💡</button>
        </div>
      </div>

      <!-- Leave Button (participants) -->
      <div id="myLeaveBtn">
        <button class="my-btn my-btn-ghost my-btn-full" style="color:#b71c1c; border-color:#ffcdd2;" onclick="myLeaveRoom()">
          ← Leave Room
        </button>
      </div>
  ...
  <script>
    ...
    function myRenderPresence(listId, items, isAdmin) {
      ...
      el.innerHTML = items.map(u =>
        '<div class="my-presence-chip' + (isAdmin ? ' admin' : '') + '">' +
        myEscape(u.name || u) + '</div>'
      ).join('');
    }
    ...
    // Check AI availability on page load (non-blocking)
