...
myIo.on('connection', (mySocket) => {
  ...
  // ── Admin: release entry to chat but KEEP in pool for AI ──
  mySocket.on('myChatKeepPoolEntry', (myData) => {
    const { room, id, name, msg } = myData;
    const myRoom = myRooms[room];
    if (!myRoom || !myRoom.admins.some(a => a.id === mySocket.id)) return;
    const entry = myRoom.pool.find(e => e.id === id);
    if (!entry) return;
    // Update text in case admin edited it
    if (msg) { entry.msg = msg; entry.text = `${entry.name}: ${msg}`; }
    // Broadcast as chat without removing from pool
    myIo.to(room).emit('myReleasedChat', { name: entry.name, msg: entry.msg });
    myLog(room, `Admin shared entry from "${entry.name}" to chat (kept in AI pool).`);
  });
  ...
  // ── Admin: share AI summary of own chat to all other live topics ──
  mySocket.on('myShareSummaryToAll', (myData) => {
    const { fromRoom, text } = myData;
    if (!myRooms[fromRoom] || !myRooms[fromRoom].admins.some(a => a.id === mySocket.id)) return;
    const otherRooms = Object.keys(myRooms).filter(r => r !== fromRoom);
    otherRooms.forEach(roomName => {
      myIo.to(roomName).emit('myCrossRoomSummary', { fromRoom, text });
      myLog(roomName, `📡 Cross-topic summary received from "${fromRoom}".`);
    });
    myLog(fromRoom, `📡 Summary shared to ${otherRooms.length} other topic(s).`);
  });
...
#myMobileWarning b { color: #d9534f; }
...
<div id="myMainLayout">
  ...
  <div>
    ...
    <div id="myAuthPanel" class="my-card">
      ...
      <div id="myAdminSetupPanel" class="my-tab-content">
        ...
        <div id="myMobileWarning">
        ⚠️ <b>High System Requirements:</b> Admin mode requires a desktop computer with a dedicated GPU and 4GB+ of free RAM to run local AI. Cell phones/tablets are not supported for Admin roles.
       </div>
      </div>
    </div>
    ...
    <div id="myAdminDashboard">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px;margin-top:6px;">
        ...
        <button id="myCrossTopicSummaryBtn" class="my-admin-btn my-admin-btn-orange" title="Run AI over this room's chat history and share the summary to all other live topics">📡 Share Summary to All Topics</button>
      </div>
...
function checkAdminEligibility() {
    ...
    const lowMemory = navigator.deviceMemory && navigator.deviceMemory < 8; // Checks if RAM < 8GB
    const warningEl = document.getElementById('myMobileWarning');
    const adminSubmitBtn = document.getElementById('myAdminLoginBtn'); // Make sure your login button has this ID

    if (isMobile) {
        warningEl.style.display = 'block';
        // Optional: Disable the button to force them to be participants
        // adminSubmitBtn.disabled = true;
        // adminSubmitBtn.innerText = "Desktop Required for Admin";
    }
}

// Run this when the user clicks the "Admin" tab
document.querySelectorAll('.my-tab').forEach(tab => {
  ...
  function myModAddEntry(id, name, msg) {
    ...
    card.innerHTML =
      ...
      '<div class="my-mod-actions">' +
        '<button class="my-mod-btn my-mod-btn-send" title="Send to chat and remove from AI pool">Send</button>' +
        '<button class="my-mod-btn my-mod-btn-ai" title="Run through AI summariser">AI</button>' +
        '<button class="my-mod-btn my-mod-btn-release" title="Show in chat but keep in AI pool">Chat+</button>' +
        '<button class="my-mod-btn my-mod-btn-del" title="Delete without broadcasting">Del</button>' +
      '</div>';
    ...
    releaseBtn.addEventListener('click', () => {
      ...
      // Chat: broadcasts to chat BUT keeps the entry in the server pool for AI summarisation
      mySocket.emit('myChatKeepPoolEntry', { room: myCurrentRoom, id, name: currentName, msg: currentMsg });
    });
  ...
  function myAppendCrossRoomSummary(fromRoom, text) {
    const box = document.getElementById('myChatBox');
    const empty = document.getElementById('myEmptyState');
    if (empty) empty.remove();
    const div = document.createElement('div');
    div.className = 'my-msg-ai';
    div.style.borderLeftColor = '#e8a020';
    div.innerHTML = '<div class="my-msg-ai-header" style="color:#e8a020;">📡 Summary from topic: ' + myEsc(fromRoom) + '</div><div class="my-msg-ai-body">' + myEsc(text) + '</div>';
    box.appendChild(div); box.scrollTop = box.scrollHeight;
  }

  // ── Cross-topic summary: AI over this room's visible chat history ──
  async function myShareSummaryToAllTopics() {
    if (!myIsAdmin) return;
    const otherTopicCount = document.getElementById('myTopicsOnlineCount') ? parseInt(document.getElementById('myTopicsOnlineCount').textContent) : 1;

    // Collect all visible chat messages from the chat box
    const box = document.getElementById('myChatBox');
    const lines = [];
    box.querySelectorAll('.my-msg-released-body, .my-msg-ai-body, .my-msg-admin-body').forEach(el => {
      const text = el.textContent.trim();
      if (text) lines.push(text);
    });
    if (lines.length === 0) { mySetAiStatus('No chat history to summarise yet.', 'error'); return; }

    const btn = document.getElementById('myCrossTopicSummaryBtn');
    btn.disabled = true; btn.textContent = '📡 Summarising…';
    mySetAiStatus('🤖 Summarising chat for cross-topic share…', 'normal');
    mySocket.emit('myAiThinking', { room: myCurrentRoom, thinking: true });

    if (!myLanguageModelSession) {
      const ok = await myInitAI();
      if (!ok) {
        btn.disabled = false; btn.textContent = '📡 Share Summary to All Topics';
        mySocket.emit('myAiThinking', { room: myCurrentRoom, thinking: false });
        return;
      }
    }

    const systemPrompt = document.getElementById('myPrompt').value ||
      'You are summarising a live brainstorming session for sharing with other groups. Write a concise 2-4 sentence summary of the key ideas and themes discussed. Be neutral and informative.';
    const fullPrompt = 'Topic: ' + myCurrentRoom + '\\n\\nSystem Instructions: ' + systemPrompt +
      '\\n\\nChat messages so far:\\n' + lines.map((l, i) => (i + 1) + '. ' + l).join('\\n');

    try {
      const result = await myLanguageModelSession.prompt(fullPrompt);
      mySocket.emit('myAiThinking', { room: myCurrentRoom, thinking: false });
      mySetAiStatus('✅ Cross-topic summary sent.', 'success');
      mySocket.emit('myShareSummaryToAll', { fromRoom: myCurrentRoom, text: result });
    } catch (err) {
      mySocket.emit('myAiThinking', { room: myCurrentRoom, thinking: false });
      mySetAiStatus('❌ AI Error: ' + err.message, 'error');
      myLanguageModelSession = null;
  ...
  mySocket.on('myCrossRoomSummary', data => myAppendCrossRoomSummary(data.fromRoom, data.text));
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('myCrossTopicSummaryBtn').addEventListener('click', myShareSummaryToAllTopics);
