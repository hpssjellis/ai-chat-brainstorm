...
// Parse the "retry in Xs" delay from a Gemini 429 error message
function myParseRetryDelay(errMessage) {
  // Matches "Please retry in 22.6s" or "retryDelay":"22s"
...
// Countdown ticker shown in the AI status bar while waiting to retry
function myRetryCountdown(roomName, seconds) {
  let remaining = seconds;
  myNotifyAdmins(roomName, 'myAiStatus', `⏳ Rate limited — retrying in ${remaining}s...`);
...
async function myHandleAISummary(myRoomName, attempt = 1) {
  const myRoom = myRooms[myRoomName];
  if (!myRoom) return;
  ...
  myLog(myRoomName, `Sending ${myRoom.pool.length} idea(s) to Gemini... (attempt ${attempt})`);
  myNotifyAdmins(myRoomName, 'myAiStatus', `⏳ Generating summary${attempt > 1 ? ` (retry ${attempt})` : ''}...`);
  ...
  try {
    ...
    if (is429 && attempt <= 3) {
      // Extract the suggested retry delay, fall back to exponential backoff
      const suggestedDelay = myParseRetryDelay(errMsg);
      const backoffDelay = suggestedDelay || Math.min(30 * attempt, 90); // 30s, 60s, 90s cap

      myLog(myRoomName, `Rate limited by Gemini (attempt ${attempt}/3). Retrying in ${backoffDelay}s...`);

      // Show live countdown to all admins
      myRetryCountdown(myRoomName, backoffDelay);

      setTimeout(() => {
        myHandleAISummary(myRoomName, attempt + 1);
      }, backoffDelay * 1000);

    } else if (is429) {
      // Exhausted retries
      myLog(myRoomName, `AI quota exhausted after ${attempt} attempts. Pool preserved for next trigger.`);
      myNotifyAdmins(myRoomName, 'myAiStatus',
        '❌ Quota exceeded — ideas preserved. Check your Gemini billing at https://ai.dev/rate-limit');
      myNotifyAdmins(myRoomName, 'myAiError', {
        type: 'quota',
        message: 'Your Gemini API key has exceeded its free-tier quota.',
        action: 'Enable billing at https://ai.google.dev/gemini-api/docs/billing or wait for daily reset.',
        poolPreserved: true,
        poolSize: myRoom.pool.length
      });
    } else {
      // Non-quota error
      myLog(myRoomName, `AI ERROR (attempt ${attempt}): ${errMsg.substring(0, 120)}`);
...
<script>
  ...
  function myLogin() {
    ...
    mySocket.emit('myAdminLogin', {
      ...
      topic, name: myUserName, duration: 60
    });
  }
  ...
  mySocket.on('myAiError', (data) => {
    ...
    box.innerHTML += \`<div style="background:#fff3e0;padding:14px;border-left:5px solid #e65100;margin:10px 0;border-radius:0 6px 6px 0;">
      <b style="color:#e65100;">⚠️ AI Quota Error</b><br>
      <span style="font-size:0.9em;">\${data.message}</span><br>
      <span style="font-size:0.85em;color:#666;">\${data.action}</span>
      \${data.poolPreserved ? '<br><span style="font-size:0.8em;color:#388e3c;">✓ Your ' + data.poolSize + ' idea(s) are preserved and will be sent when AI is available again.</span>' : ''}
    </div>\`;
    box.scrollTop = box.scrollHeight;
  });
</script>
