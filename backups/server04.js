...
async function myHandleAISummary(isTLDR) {
  ...
  try {
    ...
    const myInstruction = isTLDR 
      ...
      : `Summarize these brainstorm ideas into a professional, bulleted list for the topic: ${myTopic}. Do not use markdown (no asterisks or hashtags), just plain text formatting.`;
  ...
  <body style="font-family:sans-serif; margin:0; padding:20px; background:#f0f2f5;">
    ...
    <script>
      ...
      mySocket.on('myBroadcastSummary', (data) => {
        const myDiv = document.getElementById('myChatBox');
        const myCleanId = "id_" + Date.now();
        myDiv.innerHTML += \`
          <div style="background:#e8f0fe; padding:18px; border-left:6px solid #1a73e8; border-radius:6px; margin:10px 0; box-shadow:0 2px 5px rgba(0,0,0,0.05); position:relative;">
            <button onclick="myCopy('\${myCleanId}')" style="position:absolute; right:10px; top:10px; font-size:0.75em; padding:4px 8px; cursor:pointer;">Copy</button>
            <b>✨ AI Brainstorm Synthesis:</b><br><br>
            <div id="\${myCleanId}">\${data.text.replace(/\\n/g, '<br>')}</div>
          </div>\`;
        myDiv.scrollTop = myDiv.scrollHeight;
        myLockInput(data.wait);
      });

      window.myCopy = function(id) {
        const myTxt = document.getElementById(id).innerText;
        navigator.clipboard.writeText(myTxt);
