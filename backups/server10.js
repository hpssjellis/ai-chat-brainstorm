...
async function myHandleAISummary(myRoomName) {
  ...
  myRoom.admins.forEach(admin => myIo.to(admin.id).emit('myAiStatus', '⏳ Generating summary...'));
  ...
  try {
    ...
    myRoom.admins.forEach(admin => {
      myIo.to(admin.id).emit('myPoolUpdate', 0);
      myIo.to(admin.id).emit('myAiStatus', '✅ Summary sent!');
    });
  } catch (err) {
    myLog(myRoomName, `AI ERROR: ${err.message}`);
